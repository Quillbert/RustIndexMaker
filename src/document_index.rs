...
impl DocumentIndex {
	...
	pub fn add_all_words(&self, line: &str, num: u32) {
		self.sender.send(Message::NewJob(String::from(line), num)).unwrap();
	}
	...
	fn set_total_words(&mut self) {
		for entry in self.index.iter() {
			self.total_words = self.total_words + entry.get_occurrences();	
		}
	...
	pub fn update_info(&mut self) {
		...
		for entry in self.index.iter() {
			...
			if (ent.get_word().len() < self.shortest_meaningful_word.len() ||
				self.shortest_meaningful_word.len() == 0) &&
				DocumentIndex::is_meaningful(&ent.get_word()) &&
				ent.get_frequency() >= 0.01 {
					self.shortest_meaningful_word = String::from(&ent.get_word());
			}
			if ent.get_word().len() > self.longest_word.len() {
				self.longest_word = String::from(ent.get_word());
			}
			let frequency = ent.get_frequency();
			if frequency < least_frequency {
				self.least_frequent_word = String::from(ent.get_word());
				least_frequency = frequency;
			}
			if frequency > most_frequency {
				self.most_frequent_word = String::from(ent.get_word());
...
impl Adder {
	fn new(receiver: Arc<Mutex<mpsc::Receiver<Message>>>, map: Arc<DashMap<String, IndexEntry>>) -> Adder {
		...
		Adder {thread: Some(thread)}
	}
}

fn add_word(map: Arc<DashMap<String, IndexEntry>>, word: &str, line_num: u32) {
	let key = String::from(word);
	let mut entry = map.entry(String::clone(&key)).or_insert(IndexEntry::new(&key));
	entry.add(line_num);
}
