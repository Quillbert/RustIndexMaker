...
fn main() {
	...
	if args.len() > 1 {
		...
		io::stdin().read_line(&mut infile_name)
			.unwrap_or_else(|err| {
	...
	if args.len() > 2 {
		...
		io::stdout().flush().expect("Flush failed");
		io::stdin().read_line(&mut outfile_name)
			.unwrap_or_else(|err| {
				eprintln!("Problem parsing input: {}", err);
				process::exit(1);
			});
	...
	entries.par_sort_unstable_by_key(|key| String::from(key.get_word()));
...
fn write_to_outfile(
	...
	index: &DocumentIndex,
	file_name: &str,
	entries: Vec<dashmap::mapref::multiple::RefMulti<'_, String, IndexEntry>>) -> Result<(), Box<dyn Error>>
{
